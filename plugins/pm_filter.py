...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("𝚃𝚑𝚒𝚜 𝚗𝚘𝚝 𝚢𝚘𝚞𝚛𝚜 !!!", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def advantage_spell_chok(msg):
    query = re.sub(
        ...
        "", msg.text, flags=re.DOTALL)  # plis contribute some common words
    ...
    if not g_s:
        ...
        await k.delete()
        return
    regex = re.compile(r".*(imdb|wikipedia).*", re.DOTALL)  # look for imdb / wiki results
    gs = list(filter(regex.match, g_s))
    gs_parsed = [re.sub(
        r'\b(\-([a-zA-Z-\s])\-\simdb|(\-\s)?imdb|(\-\s)?wikipedia|\(|\)|\-|reviews|full|all|episode(s)?|film|movie|series)',
        '', i, flags=re.DOTALL) for i in gs]
    if not gs_parsed:
        reg = re.compile(r"watch(\s[a-zA-Z0-9_\s\-\(\)]*)*\|.*",
                         re.DOTALL)  # match something like Watch Niram | Amazon Prime
        for mv in g_s:
            match = reg.match(mv)
            if match:
    ...
    movielist += [(re.sub(r'(\-|\(|\)|_)', '', i, flags=re.DOTALL)).strip() for i in gs_parsed]
